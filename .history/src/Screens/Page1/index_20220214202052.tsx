...
export default ({navigation, props} : any) =>{
  ...
  const [trocar, setTrocar]=useState(<FunctionGenerica  tipo={veiculo} setSelected={setSelected} selected={selected} />);
  const [selectednome, setSelected] = useState('')


    

  useEffect(()=>{
    setTrocar(<FunctionGenerica  tipo={veiculo} setSelected={setSelected} selected={selected} />)
    setSelected('')
  },[veiculo])
