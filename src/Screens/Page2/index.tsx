...
import { DadosAPi, Description, Sequencia } from "../../Components/Textos/style";


export default ({route, navigation}: any)=>{
  const [modelo, setModelo ] =  useState([]); 
  const [selectedP2, setSelectedP2] = useState('')
  const [selectednomeP2, setSelectednomeP2] = useState('')
     ...
     <ScrollView>
       ...
       <Description>Selecione o modelo do veiculo</Description>
         ...
         {abrir ? modelo.map((modelo, key, )=> {
          ...
          return (
              <ContainerMarcas
              onPress={()=> (sel(modelo.codigo, modelo.nome ))} 
               key={key}>
               <DadosAPi>
              {modelo.nome}
              </DadosAPi>
      ...
      <ProximoP2 
       onPress={()=> navigation.navigate('Page3', {selected, veiculo, selectedP2, selectednomeP2})}
